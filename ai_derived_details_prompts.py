...
_SHARED_RULES = (
    ...
    "  - ONLY extract facts that appear explicitly in the provided source text.\n"
    "  - Do NOT invent, embellish, or fill gaps with plausible-sounding details.\n"
    "  - If the source text says NOT_DOCUMENTED for a field, output NOT_DOCUMENTED — do NOT replace it with fabricated content.\n"
    "  - Do NOT invent operation names, patrol details, enemy unit identities, weapon types, "
    "    wound descriptions, evacuation details, or combat narratives that are not in the source.\n"
    "  - Do NOT invent war diary entries, eyewitness testimony, veteran quotes, or book references.\n"
...
def get_circumstances_of_death_prompt(combined_text: str) -> tuple:
    ...
    system = (
        ...
        "  - If an Operation name is explicitly stated IN THE SOURCE TEXT, begin with:\n"
        "    \"During Operation [Operation Name], …\"\n"
        "  - Describe ONLY the manner of death as stated in the source text.\n"
        "  - Do NOT invent enemy contact type, weapon details, evacuation, or patrol narrative\n"
        "    that are not explicitly in the source text.\n"
        "  - If the source only says 'Killed in action' with no further detail, output exactly\n"
        "    that — do not elaborate.\n\n"
...
def get_all_hotlinks_prompt(combined_text: str, valid_statuses: list[str] | None = None) -> tuple:
    ...
    system = (
        ...
        "If COMBAT (KIA, DOW, enemy action) and Operation name is explicitly IN THE SOURCE TEXT: begin with \"During Operation [Name], …\". "
        "Describe ONLY the manner of death as stated in the source text. Do NOT invent enemy contact, wounds, or patrol narrative. "
        "If source only says 'Killed in action', output exactly that — do not elaborate. "
