...
### Hotlink Metadata Filtering

The side‑panel RESPONSE display includes a metadata header generated by `_make_header`
for user reference.  The header is wrapped in sentinel tags:

    **[ Created {timestamp} used {provider} $A {cost} ({secs}s) - {option} ]**
    {JSON payload}

When `_apply_hotlinks` assembles the combined text for AI field‑derivation queries
(individual hotlinks and "All Hotlinks"), it checks the first 80 characters for `**["`.
If found, the entire `**[` … `]**` block is stripped so only the JSON payload reaches
the AI.  Both the producer (`_make_header`) and consumer (`_apply_hotlinks`) live in
`update_fatalities.py`; the sentinel contract is internal but deliberately stable.

### OnThisDay Date-of-Death Filter

The Update modal search bar includes a **Use OnThisDay** date picker + checkbox.
When the checkbox is ticked:

1. The text search is cleared and disabled (mutually exclusive).
2. Two input boxes are enabled: month (1–12) and day (1–31), defaulting to today.
3. The record list is filtered to entries whose `serviceRecordAuthority.date_of_death`
   matches `MM-DD` (e.g., `07-12` matches `1969-07-12`).
4. Invalid inputs (non-numeric, out-of-range) produce no filter.

Unticking the checkbox re‑enables the text search and disables the date picker.
State is persisted to `session.json` (`on_this_day`, `on_this_day_month`, `on_this_day_day`).

Implementation: `_on_this_day_toggled`, `_on_this_day_changed`, `_apply_search`,
and `_set_locked` in `update_fatalities.py`.
